...
stringcheck byte 0 
keycheck byte 0
phrasequestionval byte 0
keyquestionval byte 0
...
outofrangeopt PROTO
phrasequestion PROTO, returnvalue : ptr byte
keyenteredquestion PROTO, keyreturnval : ptr byte


main PROC

start:
; // For when a sting / key has been entered program starts with displaymenu
; // 
cmp stringcheck, 1
jne skipstr

invoke phrasequestion, ADDR phrasequestionval
cmp phrasequestionval, 1 ; // encrypt
jne phropt2
mov useroption, 3
jmp opt3

phropt2:
cmp phrasequestionval, 2 ; // decrypt
jne phropt3
mov useroption, 4
jmp opt4

phropt3:
cmp phrasequestionval, 3 ; // enterstring
jne phropt4
mov useroption, 1
jmp opt1
...
cmp keycheck, 1
jne tomenu
invoke keyenteredquestion, ADDR keyquestionval

cmp keyquestionval, 1
jne tomenu
mov useroption, 2
jmp opt2
...
jne opt4
cmp stringcheck, 1 ; // checks if string is in system
je nextkeycheck	   ; // if yes check if key is in system
jmp stringerrorcheck ; // if not string error

nextkeycheck:
cmp keycheck, 1
je continueencrypt
jmp keyerrorcheck ; // if no key is in system, key error
...
cmp useroption, 4
jne opt5
cmp stringcheck, 1 ; // checks if a string is in system
je nextkeycheck2   ; // if yes check if key is in system
jmp stringerrorcheck ; // if not string error

nextkeycheck2:
cmp keycheck, 1
je continueendecrypt2
jmp keyerrorcheck ; if no key is in system, key error
...
stringerrorcheck: ; // Checks if a string has been entered
...
keyerrorcheck: ; // Checks if a key has been entered
...
phrasequestion PROC, returnvalue : ptr byte
;===========================
; phrasequestion PROC:
;---------------------------
; Discription: reads input for a question returns answer
; Recieves: pointer to choice
; Requires: Nothing
; Returns: choice value
.data
prompts byte "A string is already in the system, would you like to:", 0AH, 0DH, 
			 "1. Encrypt", 0AH, 0DH, 
			 "2. Decrypt", 0AH, 0DH, 
			 "3. Enter a new phrase", 0AH, 0DH,
			 "4. Continue to menu", 0AH, 0DH, 
			 "Please enter an option: ", 0d
...
keyenteredquestion PROC, keyreturnval : ptr byte
;===========================
; keyenteredquestion PROC:
;---------------------------
; Discription: reads input for a question returns answer
; Recieves: pointer to choice
; Requires: Nothing
; Returns: choice value
.data
promptk byte "A key is already in the system, would you like to: ", 0AH, 0DH, 
			 "1. Enter a new key", 0AH, 0DH, 
			 "2. Continue with current key", 0AH, 0DH, 
			 "Please enter an option: ", 0d
...
call writestring
call readhex
mov edx, keyreturnval
mov byte ptr [edx], al
call clrscr

ret
keyenteredquestion ENDP
...
; Returns: Nothing
.data
outofrangeprompt byte "Error: /* Value given is not in menu's range */", 0AH, 0DH,
					   "----------------------------------------------", 0AH, 0DH, 0d
.code

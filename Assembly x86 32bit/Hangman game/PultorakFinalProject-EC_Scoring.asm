...
;// Date:    Dec 2021
;// Description: This program creates a hangman game and tracks games played, won, and lost
;//
;// 1.  plays hangman
;// 2.  displays score
;// 3.  Exit program
...
displaygamestats PROTO, totalwins : byte, totallosses : byte, totalplays : byte
...
gamewins byte 0	   ; //
gamelosses byte 0  ; // Variables for ec option
gameplays byte 0   ; //
...
inc gamewins ; // increases game state for display
inc gameplays
jmp exitgameloop

losegameprompt:
invoke LosePrompt, ADDR pickedword
inc gamelosses ; // increases game stats for display
inc gameplays
jmp exitgameloop

exitgameloop:
invoke displaygamestats, gamewins, gamelosses, gameplays
...
mainmenu byte "===== MAIN MENU =====", 0AH, 0DH,
			  ...
			  "2. Display score", 0AH, 0DH,
			  "3. Exit program", 0AH, 0DH, 
...
displaygamestats PROC, totalwins : byte, totallosses : byte, totalplays : byte
;===========================
; displaygamestats PROC:
;---------------------------
; Discription: Prints total games won, played, and lost
; Recieves: wins, losses, and plays
; Requires: Nothing
; Returns: Nothing 
.data
statmenu1 byte "Total games played: ", 0d
statmenu2 byte "Total games won: ", 0d
statmenu3 byte "Total games lost: ", 0d

.code
call clrscr
mov edx, offset statmenu1
call writestring 
movzx eax, totalplays
call writedec ; // writes total games played to terminal
call crlf

mov edx, offset statmenu2
call writestring
movzx eax, totalwins
call writedec ; // writes total games won to terminal
call crlf

mov edx, offset statmenu3
call writestring
movzx eax, totallosses
call writedec ; // writes total games lost to terminal
call crlf 
call crlf ; // formating
call waitmsg
ret
displaygamestats ENDP
